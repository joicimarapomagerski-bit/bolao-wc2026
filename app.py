...
def extrair_secao_jogos(texto: str) -> str:
    inicio = text_idx if (text_idx := texto.find("Next matches:")) != -1 else -1
    if text_idx != -1:
        texto = texto[text_idx:]
...
def salvar_jogos_no_banco(jogos):
    ...
    cur = conn.cursor()
    for jogo in jogos:
        time_a_var = jogo["time_a"]
        ultima_atualizacao_var = jogo["ultima_atualizacao"]
        gols_real_b_var = jogo["gols_real_b"]
        cur.execute("""
        ...
        """, (
            jogo["id"], time_a_var, jogo["time_b"], jogo["data_jogo"],
            jogo["gols_real_a"], gols_real_b_var, jogo["status"],
            jogo["stage"], ultima_atualizacao_var
        ))
    conn.commit()
    conn.close()
...
def salvar_palpite(usuario, jogo_id, gols_a, gols_b):
    ...
    # MODO INVISÍVEL: Se o usuário logado for Joici, não gera linhas na tabela de logs/histórico
    ...
    # CARIMBO DE TEMPO: Grava e atualiza o data_registro explicitamente no banco
    cur.execute("""
        INSERT INTO palpites_placar (usuario, jogo_id, gols_time_a, gols_time_b, data_registro, confronto)
        VALUES (?, ?, ?, ?, ?, ?)
...
with aba_palpites:
    ...
    for jogo in jogos_ativos:
        ...
        # BLOQUEIO DINÂMICO: Bloqueia no horário se já houver palpite. Bloqueia faltando 184 segundos se não houver palpite.
        if ja_palpitou:
            foi_bloqueado = (jogo["status"] == "FT" or agora >= datetime.fromisoformat(jogo["data_jogo"]))
        else:
            segundos_restantes = (datetime.fromisoformat(jogo["data_jogo"]) - agora).total_seconds()
            foi_bloqueado = (jogo["status"] == "FT" or segundos_restantes <= 184)
        ...
        with st.container(border=True):
            ...
            with st.expander(f"📅 {datetime.fromisoformat(jogo['data_jogo']).strftime('%d/%m/%Y %H:%M')} | 📊 Ver Odds"):
...
with aba_finalizados:
    ...
    for jogo in jogos_finalizados:
        ...
        gols_real_b = Urban_gols_real_b_val := jogo["gols_real_b"]
        ...
        with st.container(border=True):
            ...
            with c_status:
                st.markdown(f"<div style='text-align: center; color: #10b981; font-size: 14px; margin-top: 0px;'><b>✅ Encerrado</b><br><span style='color: gray; font-size: 12px;'>{datetime.fromisoformat(jogo['data_jogo']).strftime('%d/%m %H:%M')}</span></div>", unsafe_allow_html=True)
            ...
            if ja_palpitou and gols_real_a is not None and gols_real_b is not None:
                ...
                text_pts = f"+{pontos} pontos" if pontos > 0 else "0 pontos"
                st.markdown(f"<div style='text-align: center; font-size: 14px;'>Seu palpite foi: <b>{pga} x {pgb}</b> &nbsp;•&nbsp; <span style='color: {cor_pontos}; font-weight: bold;'>{text_pts}</span></div>", unsafe_allow_html=True)
            elif ja_palpitou:
...
with aba_ranking:
    ...
    for usuario_nome, jogo_id, pga, pgb, conf_db in todos_palpites:
    ...
    if todos_palpites:
        for usuario_nome, jogo_id, pga, pgb, conf_db in todos_palpites:
            palpites_por_jogo.setdefault(jogo_id, []).append((usuario_nome, pga, pgb, conf_db))
        ...
        for jogo in jogos_copa:
            if jogo["id"] in palpites_por_jogo:
                ...
                jogo_bloqueado = (jogo["status"] == "FT" or agora >= datetime.fromisoformat(jogo["data_jogo"]))
                status_txt = "✅" if (jogo["status"] == "FT") else ("🔒" if jogo_bloqueado else "⏳")
                
                with st.expander(f"{flag_a} {nome_a} x {nome_b} {flag_b} | {status_txt}"):
                    for usuario_nome, pga, pgb, conf_db in palpites_por_jogo[jogo["id"]]:
                        nome_formatado = usuario_nome.title()
        ...
        for j_id, lista_p in palpites_por_jogo.items():
            if j_id.startswith("TXT_"):
                conf_fallback = lista_p[0][3]
                with st.expander(f"🔮 {conf_fallback} | 🔒 Forçado"):
                    for usuario_nome, pga, pgb, _ in lista_p:
