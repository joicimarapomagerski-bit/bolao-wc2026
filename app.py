...
def sincronizar_agenda_e_odds():
    ...
    try:
        lista_odds = buscar_odds_native_stats()
        total_atualizados = salvar_odds_no_banco(lista_odds)
        msgs.append(f"Odds OK ({total_atualizados} jogos atualizados)")
...
def calcular_pontos(gp_a, gp_b, gr_a, gr_b):
    ...
    if (gp_a > gp_b and gr_a > gr_b) or (gp_a < gp_b and gr_a < gr_b) or (gp_a == gp_b and gr_a == gr_b):
...
def salvar_palpite(usuario, jogo_id, gols_a, gols_b):
    ...
    # MODO INVISÍVEL: Se for a Joici, pula a gravação da tabela de histórico público
    ...
    # CORREÇÃO DO CARIMBO: data_registro é explicitamente modificado na tabela operacional
    cur.execute("""
...
with aba_palpites:
    ...
    for jogo in jogos_ativos:
        ...
        # PERMISSÃO EXCLUSIVA + BLOQUEIO DINÂMICO
        if usuario.strip().lower() == "joici":
            foi_bloqueado = False  # Joici edita palpites anteriores e ativos sem bloqueio temporal
        elif ja_palpitou:
            foi_bloqueado = agora >= jogo["data_jogo"]
        else:
            segundos_restantes = (jogo["data_jogo"] - agora).total_seconds()
            foi_bloqueado = segundos_restantes <= 184
        ...
        with st.container(border=True):
            ...
            with c_time_a: st.markdown(f"<div style='text-align: right; margin-top: 5px;'><b>{nome_a}</b> {flag_a}</div>", unsafe_allow_html=True)
            ...
            with c_time_b: st.markdown(f"<div style='text-align: left; margin-top: 5px;'>{flag_b} <b>{nome_b}</b></div>", unsafe_allow_html=True)
            with c_btn:
                if pode_palpitar:
            ...
            with st.expander(f"📅 {jogo['data_jogo'].strftime('%d/%m/%Y %H:%M')} | 📊 Ver Odds"):
                if jogo["odd_time_a"] is not None and jogo["odd_empate"] is not None and jogo["odd_time_b"] is not None:
                    ...
                    with c_odd1: st.metric(label=f"{flag_a} {nome_a}", value=f"{float(jogo['odd_time_a']):.2f}", delta=f"{probs['a']:.1f}%" if probs else None)
                    with c_oddx: st.metric(label="🤝 Empate", value=f"{float(jogo['odd_empate']):.2f}", delta=f"{probs['e']:.1f}%" if probs else None)
                    with c_odd2: st.metric(label=f"{flag_b} {nome_b}", value=f"{float(jogo['odd_time_b']):.2f}", delta=f"{probs['b']:.1f}%" if probs else None)
...
with aba_finalizados:
    if not jogos_finalizados: st.info("Nenhum jogo finalizado ainda.")
    for jogo in jogos_finalizados:
        pga, pgb, ja_palpitou = buscar_palpite_usuario(usuario, jogo["id"])
        with st.container(border=True):
            st.markdown(f"⚽ **{nome_time_ptbr(jogo['time_a'])} {jogo['gols_real_a']} x {jogo['gols_real_b']} {nome_time_ptbr(jogo['time_b'])}**")
            if ja_palpitou:
...
with aba_ranking:
    ...
    for u, j_id, pga, pgb, conf_db in todos_palpites:
    ...
    if todos_palpites:
        palpites_por_jogo = {}
        for usuario_nome, j_id, pga, pgb, conf_db in todos_palpites:
            palpites_por_jogo.setdefault(j_id, []).append((usuario_nome, pga, pgb, conf_db))
...
with aba_historico:
    ...
    if historico:
        for u, j_id, pga, pgb, dt_reg, conf_db in historico:
            jogo = mapa_jogos.get(j_id)
            nome_conf = conf_db if conf_db else f"Jogo ID {j_id}"
            if jogo: nome_conf = f"{nome_time_ptbr(jogo['time_a'])} x {nome_time_ptbr(jogo['time_b'])}"
