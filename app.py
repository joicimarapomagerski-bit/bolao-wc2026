...
def calcular_pontos(gp_a, gp_b, gr_a, gr_b):
    ...
    if (gp_a > gp_b and gr_a > gr_b) or (gp_a < gp_b and gr_a < gb_b := gr_b) or (gp_a == gp_b and gr_a == gr_b):
        ...
        with st.container(border=True):
            ...
            with st.expander(f"📅 {jogo['data_jogo'].strftime('%d/%m/%Y %H:%M')} | 📊 Ver Odds"):
                if jogo["odd_time_a"] is not None and jogo["odd_empate"] is not None and jogo["odd_time_b"] is not None:
                    favorito_nome, odd_favorito = determinar_favorito(nome_time_ptbr(jogo['time_a']), nome_time_ptbr(jogo['time_b']), jogo["odd_time_a"], jogo["odd_empate"], jogo["odd_time_b"])
                    ...
                    with c_odd1:
                        st.metric(label=f"{bandeira_time(jogo['time_a'])} {nome_time_ptbr(jogo['time_a'])}", value=f"{float(jogo['odd_time_a']):.2f}", delta=f"{probs['a']:.1f}%" if probs else None)
                    with c_oddx:
                        st.metric(label="🤝 Empate", value=f"{float(jogo['odd_empate']):.2f}", delta=f"{probs['e']:.1f}%" if probs else None)
                    with c_odd2:
                        st.metric(label=f"{bandeira_time(jogo['time_b'])} {nome_time_ptbr(jogo['time_b'])}", value=f"{float(jogo['odd_time_b']):.2f}", delta=f"{probs['b']:.1f}%" if probs else None)
...
with aba_historico:
    ...
    if historico:
        for u, j_id, pga, pgb, dt_reg in historico:
            jogo = mapa_jogos.get(j_id)
            nome_conf = f"Jogo ID {j_id}"
            if jogo:
